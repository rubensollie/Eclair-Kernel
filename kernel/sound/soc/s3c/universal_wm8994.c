...
#include <linux/clk.h>	// CONFIG_SND_UNIVERSAL_WM8994_MASTER
...
int univ6442_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	...
	int bfs, rfs, psr, ret;
	u32 ap_codec_clk,temp;	// CONFIG_SND_UNIVERSAL_WM8994_MASTER
	struct clk    *clk_out,*clk_epll;	// CONFIG_SND_UNIVERSAL_WM8994_MASTER
...
static int univ6442_wm8994_init(struct snd_soc_codec *codec)
{
#if 0
	debug_msg("%s\n", __FUNCTION__);
...
static int __init univ6442_audio_init(void)
{
	int ret;
...
static void __exit univ6442_audio_exit(void)
{
	debug_msg("%s\n", __FUNCTION__);
...
module_init(univ6442_audio_init);
module_exit(univ6442_audio_exit);
module_param (lowpower, int, 0444);

/* Module information */
MODULE_DESCRIPTION("ALSA SoC UNIVERSAL S5P6442 WM8994");
MODULE_LICENSE("GPL");
