...
/* for voice recognition path */
int wm8994_voice_recognition = 0; 
	...
	unsigned int value)
{
	...
	if (ret < 0)
	{
		dev_err(codec->dev, "Failed to write 0x%x:0x%x , ret %d\n", reg, value, ret);
		return ret;
	}
...
static const char *voice_recognition[] = { "Off", "ON" };
...
static int wm8994_get_voice_recognition(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = wm8994_voice_recognition;
	return 0;
}

static int wm8994_set_voice_recognition(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	wm8994_voice_recognition = ucontrol->value.integer.value[0];
	printk("wm8994_set_voice_recognition : %d\n", wm8994_voice_recognition);
...
static const struct soc_enum path_control_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(voice_recognition),voice_recognition),
};
...
static const struct snd_kcontrol_new wm8994_snd_controls[] = {
    ...
    SOC_ENUM_EXT("Down Gain", path_control_enum[7], wm8994_get_voice_recognition, wm8994_set_voice_recognition),
} ;//snd_ctrls
