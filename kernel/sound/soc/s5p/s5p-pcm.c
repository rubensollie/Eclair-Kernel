	...
	struct snd_pcm_hw_params *params)
{
	...
	if(totbytes != MAX_LP_BUFF){
		s5pdbg("Use full buffer(128KB) in lowpower playback mode!");
		return -EINVAL;
	}
	...
	s5pdbg("DmaAddr=@%x Total=%lubytes PrdSz=%u #Prds=%u\n",
...
static void s5p_pcm_setmode(int lpmd, void *ptr)
{
	...
	if(s3c_pcm_pdat.lp_mode){
		...
		s3c_pcm_pdat.pcm_pltfm.pcm_ops->hw_params = s5p_pcm_hw_params;
		s3c_pcm_pdat.pcm_pltfm.pcm_ops->hw_free = s5p_pcm_hw_free;
		s3c_pcm_pdat.pcm_pltfm.pcm_ops->prepare = s5p_pcm_prepare;
