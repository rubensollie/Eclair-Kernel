...
*/
		...
		s5pdbg("Setting FOUTepll to %dHz", s3c_i2s.clk_rate);
		clk_put(clk);
		break;
#endif
...
// begin : CONFIG_SND_WM8994_MASTER_MODE
		...
		clk_put(clk);
// end : CONFIG_SND_WM8994_MASTER_MODE
...
//	iisahb  = S3C_IISAHB_DMARLD | S3C_IISAHB_DISRLDINT;
	...
	/* Enable interrupts (don't use rx & tx INT for apollo) */
	iiscon &= ~S3C_IISCON_FRXOFINTEN;
	// iiscon |= S3C_IISCON_FRXOFINTEN;	/* don't use RX Over-run INT */
	...
	if(s3c_i2s_pdat.lp_mode){
		...
		iiscon &= ~S3C_IISCON_FTXSURINTEN;
		// iiscon |= S3C_IISCON_FTXURINTEN;	/* don't use TX Under-run INT */
		iismod &= ~S3C_IISMOD_TXSLP;
		iisahb = 0;
	}
			     ...
			     struct snd_soc_dai *dai)
{
	...
	s5pdbg("Got Audio Bus Clock -> %s\n", EXTCLK);

#ifndef CONFIG_SND_WM8994_MASTER_MODE
	
	// [i2s : master, wm8994 : slave]
	...
	s5pdbg("Set fout_epll as parent of %s\n", EXTPRNT);

#else
			
	// [i2s : slave, wm8994 : master]

	cm = clk_get(NULL, "fout_epll");
	if (IS_ERR(cm)) {
		printk("failed to get fout_epll\n");
        goto lb2;
	}
    //s5pdbg("Got fout_epll\n");

	cf = clk_get(NULL, "audss_hclk");
    if (IS_ERR(cf)) {
    	printk("failed to get audss_hclk\n");
		goto lb1;
	}
	//s5pdbg("Got audss_hclk\n");
...
static void s5p_i2s_setmode(int lpmd)
{
