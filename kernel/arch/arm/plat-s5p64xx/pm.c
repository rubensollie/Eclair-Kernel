...
//	s5p6442_pm_configure_extint();	//sm.kim: remove wrong configuration
...
#ifdef WAKELOCK_TEMP
	wake_lock_init(&pm_wake_lock, WAKE_LOCK_SUSPEND, "pm");
#endif /* WAKELOCK_TEMP */
#ifdef CONFIG_S5P64XX_POWER_GATING
	s5p6442_pwrgate_init();   // It includes the clk gate initializations.
#else
