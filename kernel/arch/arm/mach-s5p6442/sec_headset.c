...
static void headset_input_selector(int headset_type_status)
{
	...
	switch(headset_type_status)
	{
		...
		{
			printk(KERN_INFO "unknown headset device attached. User must select headset device type\n");
			break;
		}
		default :
...
//	gpio_set_value(GPIO_ADC_EN,1); // ADC_EN
	...
	if (state)
	{
		...
		if(adc < 5)
		{
			printk(KERN_INFO "3 pole headset attatched : adc = %d\n", adc);
#if (CONFIG_BOARD_REVISION != CONFIG_APOLLO_EMUL)
//			gpio_set_value(GPIO_MICBIAS2_EN, 0);
		...
		{
			printk(KERN_INFO "4 pole  headset attached : adc = %d\n", adc);			
...
/*		else if(adc >=2500 && adc <2700)
		...
		{
			printk(KERN_INFO "headset detected but unknown device : adc = %d\n", adc);
#if (CONFIG_BOARD_REVISION != CONFIG_APOLLO_EMUL)
//			gpio_set_value(GPIO_MICBIAS2_EN, 0);
#endif
	...
	msleep(500);
	wake_unlock(&headset_sendend_wake_lock);
//	wake_lock_timeout(&headset_sendend_wake_lock, HZ);
}
...
static void headset_detect_change(struct work_struct *ignored)
{
	...
	else if(!state && (current_headset_type_status != SEC_HEADSET_NO_DEVICE))
	{
		current_headset_type_status = SEC_HEADSET_NO_DEVICE;
		switch_set_state(&switch_headset_detection, current_headset_type_status);
		printk(KERN_INFO "HEADSET dev detached %d \n", send_end_irq_token);			
...
//			schedule_delayed_work(&ear_adc_cal_work, 200);
			...
			printk(KERN_ERR "wrong headset_detect_timer_token count %d", headset_detect_timer_token);
	}
	else
		printk(KERN_INFO "headset detach!! %d", headset_detect_timer_token);
}
...
static void send_end_key_event_timer_handler(unsigned long arg)
{
	...
	if(headset_state && sendend_state)
	{
		...
		{
			printk(KERN_INFO "SEND/END is pressed\n");
			input_report_key(hi->input, KEYCODE_SENDEND, 1);
			input_sync(hi->input);
			send_end_key_timer_token = 0;
		}
		else
			printk(KERN_ERR "[HEADSET]wrong timer counter %d\n", send_end_key_timer_token);
	}else
			printk(KERN_ERR "[HEADSET]GPIO Error\n");
}
...
static ssize_t select_headset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	{
		printk(KERN_ERR "Error : mic bias enable complete but headset detached!!\n");
		current_headset_type_status = SEC_HEADSET_NO_DEVICE;
		gpio_set_value(GPIO_MICBIAS_EN, 0);
	}
