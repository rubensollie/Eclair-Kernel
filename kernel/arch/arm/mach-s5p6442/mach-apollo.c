...
static void display_cfg_gpio(struct platform_device *pdev)
{
	...
	s3c_gpio_cfgpin(GPIO_DISPLAY_SO, S3C_GPIO_SFN(GPIO_DISPLAY_SO_STATE));	/* SPO Data */
	s3c_gpio_setpull(GPIO_DISPLAY_SO, S3C_GPIO_PULL_UP);
...
#if defined(CONFIG_FB_S3C_LMS300)
static int lms300_backlight_on(struct platform_device *pdev)
{
        int err;
        ...
        if (err) {
                printk(KERN_ERR "failed to request GPF0 for "
                        "lcd backlight control\n");
                return err;
        }

        //If you want to turn on LCD backlight, must make CODEC_LDO_EN high
        //gpio_direction_output(GPIO_CODEC_LDO_EN, GPIO_LEVEL_HIGH);
        
        gpio_direction_output(GPIO_MLCD_ON, GPIO_LEVEL_HIGH);
        gpio_free(GPIO_MLCD_ON);

        return 0;
}

static int lms300_reset_lcd(struct platform_device *pdev)
{
	int err;

	err = gpio_request(GPIO_ALS_nRST, "GPF1");
	if (err) {
		printk(KERN_ERR "failed to request GPF1 for "
			"lcd reset control\n");
		return err;
	}

	gpio_direction_output(GPIO_ALS_nRST, GPIO_LEVEL_HIGH);
	mdelay(3);
...
#endif //defined(CONFIG_FB_S3C_LMS300)

static int s6d04d1_backlight_on(struct platform_device *pdev)
{
        int err;
...
static struct s3c_platform_fb lms300_data __initdata = {
	.hw_ver	= 0x62,
	...
	.backlight_on = lms300_backlight_on,
	.reset_lcd = lms300_reset_lcd,
#else
...
static struct spi_board_info spi_board_info[] __initdata = {
		...
		.chip_select	= 0,
		.mode		= SPI_MODE_3,
		.controller_data = (void *)LCD_CS,
	},
};

#define LCD_CLK	GPIO_DISPLAY_CLK
#define LCD_SI	GPIO_DISPLAY_SI
static struct spi_gpio_platform_data lms300_spi_gpio_data = {
	.sck	= LCD_CLK,
	.mosi	= LCD_SI,
	.miso	= -1,
	.num_chipselect	= 1,
...
static struct platform_device s3c_device_spi_gpio = {
	...
	.dev	= {
		...
		.platform_data	= &lms300_spi_gpio_data,
	},
};
#endif
...
int apollo_get_saved_lcd_id_value()
{
...
int apollo_get_hw_rev_pin_value()
{
...
static struct s3c_ts_mach_info s5p6442_ts_info __initdata = {
	.adcts = {
		.delay = 0xFF,
		.presc = 49,
	...
	.sampling_time = 18,
	.remove_max_min_sampling = true,
	.sampling_interval_ms = 20,
	.screen_size_x = 240,
	.screen_size_y = 400,
	.use_tscal = true,
	.tscal = {119, -6710, 21131008, 8758, 38, -5081680, 65536},
};
#endif
...
/*VNVS:START 16-NOV'09 ---- added i2c7_platdata for FM Radio I2C path*/
...
static void apollo_cam_pin_config(void)
{
	...
	/* These codes are using for GPIO. it should be used to SCL and SDA*/
...
static void apollo_pm_power_off(void)
{
	...
	while (1) { printk("MIDAS@%s: System Fault\n", __func__); }
}
...
static void apollo_phone_reset_gpio_drvstr_set()
{
...
#endif
#ifdef CONFIG_TOUCHSCREEN_S3C
	s3c_ts_set_platdata (&s5p6442_ts_info);
#endif
...
//	s3cfb_set_platdata(&fb_data);
	...
	ret = platform_add_devices(smdk6442_devices, ARRAY_SIZE(smdk6442_devices));
	
/* 
 * Registering the proximity sensor depending on the HW Revision
 * Capella CM3607 for HW_REV<=4, Sharp GP2A for HW_REV>4
 */
	if(!ret)
	{
		if(apollo_get_remapped_hw_rev_pin()<=4)
			platform_device_register(&sec_device_proximity);
		else
		{
...
#define ONEDRAM_IS_USED_FOR_RESERVED_MEM_ONLY
...
#ifdef ONEDRAM_IS_USED_FOR_RESERVED_MEM_ONLY
	mi->nr_banks = 1;
#else
	printk("OneDRAM unreserved size:%d\n", PHYS_UNRESERVED_SIZE);
...
/********************************* ALIVE PART ************************************************/
	...
	{ GPIO_ACC_INT, 0x0, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
	...
	{ GPIO_RESET_REQ_N, 0x0, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
	...
	{ GPIO_MSENSE_IRQ, 0x0, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
...
/********************************* ALIVE PART ************************************************/
	...
	{ GPIO_PS_OUT, 0xf, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
	{ GPIO_KEY_ROW2, 0xf, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},  // CAPTURE
	{ GPIO_KEY_ROW3, 0x0, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},  // NC
	{ S5P64XX_GPH3(4), 0xf, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},  // VOL_UP
	{ S5P64XX_GPH3(5), 0xf, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},  // VOL_DOWN
//	{ S5P64XX_GPH3(6), 0x1, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
//	{ S5P64XX_GPH3(7), 0x1, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE},
...
static int s5p6442_gpio_table[][6] = {
	...
	{ GPIO_AP_COM_RXD, GPIO_AP_COM_RXD_STATE, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
	...
	{ GPIO_TOUCH_RST, 1, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
...
//	{GPIO_UART_SEL, 1, GPIO_LEVEL_NONE, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE }, // check
	...
	/* GPG1 */
	{ GPIO_WLAN_SDIO_CLK, 0, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
	{ GPIO_WLAN_SDIO_CMD, 0, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
	{ GPIO_WLAN_nRST, GPIO_WLAN_nRST_STATE, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
	...
	{ GPIO_FM_INT, GPIO_FM_INT_STATE, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
	{ GPIO_FM_BUS_nRST, GPIO_FM_BUS_nRST_STATE, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
	...
	/*GPJ3 */
	{ GPIO_WLAN_BT_EN, 0, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE},
...
//	/* NC */{S5P64XX_GPJ3(6), 1, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_OUT0, S3C_GPIO_PULL_NONE },
	{ GPIO_BT_nRST, 0, GPIO_LEVEL_NONE, S3C_GPIO_PULL_DOWN, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
#ifdef USE_GPIO_ENABLE_FOR_LCD_POWER
...
//	/* NC */{S5P64XX_MP04(6), 1, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_OUT0, S3C_GPIO_PULL_NONE },
	...
	/* NC */{S5P64XX_MP05(6), 1, GPIO_LEVEL_LOW, S3C_GPIO_PULL_NONE, S3C_GPIO_SLP_INPUT, S3C_GPIO_PULL_NONE },
...
void s5p6442_init_gpio(void)
{
	...
	__raw_writel(0x20, (S5P64XX_GPA0_BASE + 0x2d0));  // for reducing sleep current (MP0_2)
	__raw_writel(0x290, (S5P64XX_GPA0_BASE + 0x2f0));  // for reducing sleep current (MP0_3)
