...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	host->mrq = mrq;
	if ((host->mmc->caps & MMC_CAP_ON_BOARD) || (host->flags & SDHCI_DEVICE_ALIVE))
...
static void sdhci_tasklet_card(unsigned long param)
{
	...
	if (!(readl(host->ioaddr + SDHCI_PRESENT_STATE) & SDHCI_CARD_PRESENT)  || !(host->flags & SDHCI_DEVICE_ALIVE)) {
		if (host->mrq) {
	...
	if (wakeup_sdslot) {
		wakeup_sdslot = 0;
		if (!(host->flags & SDHCI_DEVICE_ALIVE))
			return;
	}
...
//	spin_lock(&host->lock);
	mdelay(1);
	eint_num = irq - IRQ_EINT(16);
	__raw_writel((1 << eint_num), S5P64XX_EINT2PEND);


	//__raw_writel(eint_irq_to_bit(IRQ_EINT(8)), S5P64XX_EINTPEND(eint_pend_reg(IRQ_EINT(8))));

	ext_CD_int = __raw_readl(S5P64XX_GPH2DAT);
	ext_CD_int &= 0x1;	/* GPH2[0] */
	
	if (!strcmp("mmc1", mmc_hostname(host->mmc)))
		printk(KERN_DEBUG "%s: %s state-%d\n", 
					mmc_hostname(host->mmc), __func__, ext_CD_int);
...
#endif
		host->flags |= SDHCI_DEVICE_ALIVE;
		eint0msk = __raw_readl(S5P64XX_EINT2MASK);
		eint0msk |= (1 << eint_num);
		__raw_writel(eint0msk, S5P64XX_EINT2MASK);
	}
	else {
		host->flags &= ~SDHCI_DEVICE_ALIVE;
		host->mmc->caps &= ~MMC_CAP_ON_BOARD;
...
{
	int ret;
	struct sdhci_s3c *ourhost = sdhci_priv(host);
...
static int __init sdhci_drv_init(void)
{
	printk(KERN_INFO DRIVER_NAME
		": Samsung S5PC6442 SD/MMC driver\n");
	return 0;
}
