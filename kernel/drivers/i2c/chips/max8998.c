...
static unsigned short MAX8998_normal_i2c[] = { I2C_CLIENT_END };
static unsigned short MAX8998_ignore[] = { I2C_CLIENT_END };
static unsigned short MAX8998_probe[] = { I2C_GPIO3_DEVICE_ID,
	(I2C_SLAVE_ADDR_PM_MAX8998 >> 1), I2C_CLIENT_END };

static struct i2c_client_address_data MAX8998_addr_data = {
	.normal_i2c = MAX8998_normal_i2c,
	.ignore		= MAX8998_ignore,
	.probe		= MAX8998_probe,
};
...
unsigned int pmic_read(u8 slaveaddr, u8 reg, u8 *data, u8 length)
{
	...
	if (MAX8998_read(client, reg, data, length) < 0)
	{ 
...
unsigned int pmic_write(u8 slaveaddr, u8 reg, u8 *data, u8 length)
{
	...
	if (MAX8998_write(client, reg, data, length) < 0) { 
		pr_err(KERN_ERR "%s -> Failed! (reg 0x%02x, data 0x%02x)\n", __func__, reg, *data);
		return PMIC_FAIL;
	}	
...
static int MAX8998_attach(struct i2c_adapter *adap, int addr, int kind)
{
	struct i2c_client *c;
	int ret = 0;
	byte stat2;
	...
	strcpy(c->name, MAX8998_driver.driver.name);
	c->addr = addr;
	c->adapter = adap;
	c->driver = &MAX8998_driver;

	if ((ret = i2c_attach_client(c)))
		goto error;

	MAX8998_i2c_client = c;
	printk("################ %s : Success \n", __FUNCTION__);
...
#ifdef CONFIG_MACH_APOLLO
	ret = led_classdev_register(&adap->dev, &apollo_button_backlight_led);
	if (ret < 0)
...
static int MAX8998_attach_adapter(struct i2c_adapter *adap)
{
	return i2c_probe(adap, &MAX8998_addr_data, MAX8998_attach);
}


static int MAX8998_detach_client(struct i2c_client *client)
{
...
static struct i2c_driver MAX8998_driver = {
	...
	.attach_adapter = MAX8998_attach_adapter,
	.detach_client = MAX8998_detach_client,
...
{
	int ret;
	ret = i2c_add_driver(&MAX8998_driver);
	pmic_init_status = 1;
	return ret;
}
