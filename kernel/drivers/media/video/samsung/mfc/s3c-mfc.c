...
static int s3c_mfc_probe(struct platform_device *pdev)
{
	...
	if (MFC_HW_Init() == FALSE)
		return -ENODEV;
	printk("###############%s:misc_register:before \n", __FUNCTION__);
	ret = misc_register(&s3c_mfc_miscdev);
	printk("###############%s:misc_register:after \n", __FUNCTION__);
...
//	clk_enable(mfc_pclk);
	...
	mfc_pwr = readl(S5P_NORMAL_CFG);
	mfc_pwr |= (1<<9);
	__raw_writel(mfc_pwr, S5P_NORMAL_CFG);


	// 2. Check MFC power on
	do {
		domain_v_ready = readl(S5P_BLK_PWR_STAT);
