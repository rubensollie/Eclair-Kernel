...
static int s3c_keypad_show_key_pressed(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct s3c_keypad *keypad = (struct s3c_keypad *)dev->driver_data;
...
#ifdef __KEYPAD_PSEUDO_KYECODE__
	for(i=0; i<ARRAY_SIZE(pseudo_keycode); i++)
		{
		DPRINTK(": setbit(pseudo) %d\n", pseudo_keycode[i]);
		input_set_capability(input_dev, EV_KEY, pseudo_keycode[i]);
		}
#endif
