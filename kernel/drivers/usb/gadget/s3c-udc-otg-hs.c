...
#define USING_MACH_OTG_PHY  1

/* 6442 USB operation mode is the dedicated fifo */
#define OTG_DEDICATED_FIFO 	1

/* Set delaying or not for serial usb bulk transfer */
#define SERIAL_TRANSFER_DELAY 	0

/*
 * S/W walk-around to preventing from missing SETUP packet
 * There is rarely timing issues between Host and device on Vista with service pack 1
 * Windows app. - Windows driver - Windows Host driver - USB - USB driver - Gadget driver - Linux app.
 */
#define	ENABLE_WA_MISSING_SETUP 0
...
#ifdef DEBUG_S3C_UDC_LOCK
#define DEBUG_LOCK(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_LOCK(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_ERROR
#define DEBUG_ERROR(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_ERROR(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_SETUP
#define DEBUG_SETUP(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_SETUP(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_PM
#define DEBUG_PM(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_PM(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_EP0
#define DEBUG_EP0(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_EP0(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC
#define DEBUG(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_ISR
#define DEBUG_ISR(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_ISR(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_OUT_EP
#define DEBUG_OUT_EP(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_OUT_EP(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_IN_EP
#define DEBUG_IN_EP(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_IN_EP(fmt,args...) do {} while(0)
#endif

#ifdef DEBUG_S3C_UDC_SETUP_FEATURE
#define DEBUG_FEATURE(fmt,args...) printk("[USB] " fmt, ##args)
#else
#define DEBUG_FEATURE(fmt,args...) do {} while(0)
#endif

#define S3C_UDC_LOCK(lock) \
{	DEBUG_LOCK("[%s] spin_lock \n", __func__);\
	spin_lock(lock);\
}

#define S3C_UDC_UNLOCK(lock) \
{	DEBUG_LOCK("[%s] spin_unlock \n\n", __func__);\
	spin_unlock(lock);\
}

#define S3C_UDC_LOCK_IRQ(lock) \
{	DEBUG_LOCK("[%s] spin_lock_irq \n", __func__);\
	spin_lock_irq(lock);\
}

#define S3C_UDC_UNLOCK_IRQ(lock) \
{	DEBUG_LOCK("[%s] spin_unlock_irq \n\n", __func__);\
	spin_unlock_irq(lock);\
}

#define S3C_UDC_LOCK_IRQSAVE(lock, flags) \
{	DEBUG_LOCK("[%s] spin_lock_irqsave \n", __func__);\
	spin_lock_irqsave(lock, flags);\
}

#define S3C_UDC_UNLOCK_IRQRESTORE(lock, flags) \
...
/* 	in case of rndis, will be chaned at the time of SET_CONFIGURATION */
	...
	else if (strcmp(driver->driver.name, "android_adb_ums_acm_eth") == 0)
		dev->config_gadget_driver = ANDROID_ADB_UMS_ACM_ETH_RNDIS;	
		
	else if (strcmp(driver->driver.name, "android_adb_ums_acm_rndis_mtp") == 0)
		dev->config_gadget_driver = ANDROID_ADB_UMS_ACM_ETH_RNDIS_MTP;	
...
static struct s3c_udc memory = {
	...
	.ep[0] = {
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(0),
		  },
	...
	.ep[1] = {
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(1),
		  },
	...
	.ep[2] = {
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(2),
		  },
	...
	.ep[3] = {				
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(3),
		  },
	...
	.ep[4] = {
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(4),
		  },
	...
	.ep[5] = {
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(5),
		  },
	...
	.ep[6] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(6),
		  },
	...
	.ep[7] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(7),
		  },
	...
	.ep[8] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(8),
		  },
	...
	.ep[9] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(9),
		  },
	...
	.ep[10] = {				
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(10),
		  },
	...
	.ep[11] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(11),
		  },
	...
	.ep[12] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(12),
		  },
	...
	.ep[13] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(13),
		  },
	...
	.ep[14] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(14),
		  },
	...
	.ep[15] = {				 
		  ...
		  .fifo = (unsigned int) S3C_UDC_OTG_EP_FIFO(15),
		  },
};
//---------------------------------------------------------------------------------------
