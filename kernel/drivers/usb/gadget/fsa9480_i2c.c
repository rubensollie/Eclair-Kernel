...
{
	if (indicator_dev.dev->driver_data != &indicator_dev) {
		printk("UsbIndicator:0x%08x\n", (unsigned int)indicator_dev.print_name);
		printk("driver_data adddr : 0x%08x\n", (unsigned int)indicator_dev.dev->driver_data);
		printk("indicator_dev : 0x%08x\n", (unsigned int)&indicator_dev);

		dev_set_drvdata(indicator_dev.dev, &indicator_dev);
	}

	printk("UsbIndicator(%s):%d,c:%d\n", indicator_dev.dev->bus_id, state,indicator_dev.state);
...
int fsa9480_read(struct i2c_client *client, u8 reg, u8 *data)
{
	...
	if(!fsa9480_init)
	{
		printk(KERN_ERR "%s: fsa9480 is not initialized yet!!\n", __func__);
		return -1;
	}
...
//			DEBUG_FSA9480("[FSA9480] 4 :Manual1 (0x%x), usb_power (%d)\n ", manual1, usb_power);
			...
			fsa9480_write(&fsa9480_i2c_client, REGISTER_MANUALSW1, 0x24); // PDA Port
		...
		{
			fsa9480_write(&fsa9480_i2c_client, REGISTER_MANUALSW1, 0x90); // V_Audio port (Modem USB)
...
static int fsa9480_codec_probe(struct i2c_adapter *adap, int addr, int kind)
{
	...
	fsa9480_read(&fsa9480_i2c_client, REGISTER_MANUALSW1, &pData);
...
static int fsa9480_i2c_attach(struct i2c_adapter *adap)
{
	int ret;
	DEBUG_FSA9480("[FSA9480] %s\n ", __func__);
	ret = i2c_probe(adap, &fsa9480_addr_data, fsa9480_codec_probe);
	return ret;
}
